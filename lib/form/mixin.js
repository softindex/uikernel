...
var FormMixin = {
   ...
   */
  initForm: function initForm(settings) {
    var _this = this;
      ...
      return _regenerator2.default.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              _this._initState(settings);

              if (_this.state._formMixin.data) {
                _context.next = 21;
                break;
              }
            ...
            case 15:
              ...
              _this.state._formMixin.globalError = err;
              _context.next = 19;
            ...
            case 20:
              ...
              _this.state._formMixin.data = data;

            case 21:

              _this.state._formMixin.model.on('update', _this._handleModelChange);
              _context.next = 24;
              return _this.setState(_this.state);
  ...
  set: function set(data, validate, cb) {
    ...
    if (this.state._formMixin.autoSubmit) {
      this.submit(function (err, result) {
        _this2.state._formMixin.autoSubmitHandler(err, result);
        if (typeof cb === 'function') {
          cb(err, result);
        }
      });
  ...
  _runValidator: function _runValidator(validator, getData, output, cb) {
    ...
    validator.isValidRecord(data).then(function (validErrors) {
      if (!_this3._isUnmounted && _utils2.default.isEqual(data, getData())) {
        _this3.state._formMixin[output] = validErrors;
      }
      cb();
    }).catch(function (err) {
      if (!_this3._isUnmounted && _utils2.default.isEqual(data, getData())) {
        _this3.state._formMixin[output].clear();
      }
      cb(err);
    });
  },
