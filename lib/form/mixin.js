...
var FormMixin = {
  getInitialState: function getInitialState() {
    this._validateForm = _utils.default.throttle(this._validateForm);
   ...
   */
  initForm: (0, _callbackify.default)(
  /*#__PURE__*/
  function () {
  ...
  hasChanges: function hasChanges(field) {
    ...
    if (field === undefined) {
      return !_utils.default.isEmpty(state.changes);
    }
  ...
  hasError: function hasError(field) {
    ...
    if (state.partialErrorChecking) {
      if (!state.changes.hasOwnProperty(field) || _utils.default.isEqual(state.changes[field], state.data[field])) {
        return false;
      }
    }
  ...
  getData: function getData() {
    ...
    return _utils.default.cloneDeep(this._getData());
  },
  ...
  getValidationErrors: function getValidationErrors() {
    ...
    if (this.state._formMixin.partialErrorChecking) {
      ...
      for (field in this.state._formMixin.data) {
        ...
        if (!this.state._formMixin.changes.hasOwnProperty(field) || _utils.default.isEqual(this.state._formMixin.changes[field], this.state._formMixin.data[field])) {
  ...
  updateField: function updateField(field, value) {
    ...
    this.set((0, _defineProperty2.default)({}, field, _utils.default.parseValueFromEvent(value)));
  },
  validateField: function validateField(field, value, cb) {
    this.set((0, _defineProperty2.default)({}, field, _utils.default.parseValueFromEvent(value)), true, cb);
  },
  ...
  set: function set(data, validate, cb) {
    ...
    var state = this.state._formMixin;
    state.changes = _utils.default.getRecordChanges(state.model, state.data, state.changes, data);
        ...
        switch (_context2.prev = _context2.next) {
          ...
          case 20:
            ...
            if (err) {
              ...
              _utils.default.forEach(changes, function (value, field) {
                if (_utils.default.isEqual(value, newChanges[field])) {
  ...
  _handleModelChange: function _handleModelChange(changes) {
    Object.assign(this.state._formMixin.data, _utils.default.cloneDeep(changes));
  ...
  _initState: function _initState(settings) {
    ...
    this.state._formMixin = {
      ...
      warningsValidator: settings.warningsValidator || new _common.default(),
  ...
  _runValidator: function _runValidator(validator, getData, output, cb) {
    ...
    validator.isValidRecord(data).then(function (validErrors) {
      if (!_this2._isUnmounted && _utils.default.isEqual(data, getData())) {
  ...
  _getChanges: function _getChanges() {
    ...
    return _utils.default.clone(this.state._formMixin.changes);
  },
  _isDependentField: function _isDependentField(field) {
    var state = this.state._formMixin;
    return state.changes.hasOwnProperty(field) && _utils.default.isEqual(state.changes[field], state.data[field]);
  }
};
