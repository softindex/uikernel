...
export type ValidationFunction<TValue, TAsync extends 'async' | 'sync'> = (value: TValue) => ValidationResult<string | undefined, TAsync>;
export type GroupValidationFunction<TRecord extends Record<string, unknown>, TEditableField extends keyof TRecord & string, TAsync extends 'async' | 'sync'> = (record: TRecord, errors: ValidationErrors<TEditableField>) => ValidationResult<void, TAsync>;
export type GroupValidators<TRecord extends Record<string, unknown>, TEditableField extends keyof TRecord & string, TGroupValidators extends (keyof TRecord & string)[], TAsync extends 'async' | 'sync'> = {
    [TIndex in keyof TGroupValidators]: {
        ...
        fn: GroupValidationFunction<Pick<TRecord, TGroupValidators[TIndex]>, TGroupValidators[TIndex] & TEditableField, TAsync>;
    };
};
