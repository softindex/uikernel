...
declare class DeprecatedValidator<TRecord extends Record<string, unknown>, TAsyncGroupValidators extends (keyof TRecord & string)[] = [], TGroupValidators extends (keyof TRecord & string)[] = []> implements IValidator<TRecord, keyof TRecord & string> {
    ...
    fields<TField extends keyof TRecord & string>(fields: ArrayWithAtLeastOneElement<TField>, groupValidationFunction: GroupValidationFunction<TRecord, TField, keyof TRecord & string, 'sync'>): DeprecatedValidator<TRecord, TAsyncGroupValidators, [...TGroupValidators, TField]>;
    ...
    asyncFields<TField extends keyof TRecord & string>(fields: ArrayWithAtLeastOneElement<TField>, groupValidationFunction: GroupValidationFunction<TRecord, TField, keyof TRecord & string, 'async'>): DeprecatedValidator<TRecord, [...TAsyncGroupValidators, TField], TGroupValidators>;
    getValidationDependency(fields: (keyof TRecord & string)[]): (keyof TRecord & string)[];
    isValidRecord(record: OptionalRecord<TRecord>, additionalValues?: Partial<TRecord>): Promise<ValidationErrors<keyof TRecord & string>>;
}
