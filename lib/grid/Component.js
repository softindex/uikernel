...
class GridComponent extends react_1.default.Component {
    ...
    async validateRow(recordId) {
        (0, assert_1.assertNonNullish)(this.props.model, ERROR_MESSAGE_MODEL_UNAVAILABLE);
        const errors = await this.checkValidatorErrors(recordId, async (changes, recordId) => {
            if (!this.props.model) {
                return new ValidationErrors_1.default();
            }
            return await this.props.model.isValidRecord(changes, recordId);
        }, this.getRecordChanges, 'errors');
        this.setState({ errors });
    }
    async checkWarnings(recordId) {
        ...
        const warnings = await this.checkValidatorErrors(recordId, (record) => warningsValidator.isValidRecord(record), (recordId) => {
            var _a;
