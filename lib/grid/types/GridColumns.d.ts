...
export type GridGetCell<TRecord extends Record<string, unknown>> = (recordWithChanges: TRecord, selected: boolean, initialRecord: Partial<TRecord>, gridRef: IGridRef<any, TRecord, any, any, boolean> | undefined) => string;
export type GridCellRender<TRecord extends Record<string, unknown>> = readonly [
    ...(string & keyof TRecord)[],
    GridGetCell<TRecord>
];
type GridEditorConfig<TColumnId extends string, TRecord extends Record<string, unknown>, TEditorField extends string & keyof TRecord> = {
    editor?: undefined;
    editorField?: undefined;
} | ({
    editor: (this: EditorContext<TRecord, TEditorField>, record: TRecord, gridRef: IGridRef<any, TRecord, any, any, boolean>) => JSX.Element | null;
} & (TColumnId extends TEditorField ? {
    editorField?: TEditorField;
} : {
    editorField: TEditorField;
}));
export type GridColumnName<TKey, TRecord extends Record<string, unknown>> = string | ((gridRef: IGridRef<TKey, TRecord, any, any, boolean>) => React.ReactNode);
export type GridColumnConfig<TColumnId extends string, TRecord extends Record<string, unknown>, TEditorField extends string & keyof TRecord, TKey, TElement extends HTMLElement = HTMLElement> = {
    className?: string;
    escape?: boolean;
