...
function getRecordChanges(model, data, changes, newChanges) {
  ...
  var changedFields = Object.keys(result);

  for (var _i5 = 0, _changedFields = changedFields; _i5 < _changedFields.length; _i5++) {
    var fieldName = _changedFields[_i5];
  ...
  Object.assign(result, pick(data, model.getValidationDependency(changedFields)));
  return result;
}
